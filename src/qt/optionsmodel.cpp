...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", InvoiceCoinUnits::IVC);
    ...
    if (!settings.contains("nAnonymizeInvoiceCoinAmount"))
        settings.setValue("nAnonymizeInvoiceCoinAmount", 1000);
    nAnonymizeInvoiceCoinAmount = settings.value("nAnonymizeInvoiceCoinAmount").toLongLong();
    if (settings.contains("nMNengineRounds"))
        SoftSetArg("-mnenginerounds", settings.value("nMNengineRounds").toString().toStdString());
    if (settings.contains("nAnonymizeInvoiceCoinAmount"))
        SoftSetArg("-anonymizeInvoiceCoinamount", settings.value("nAnonymizeInvoiceCoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeInvoiceCoinAmount:
            return QVariant(nAnonymizeInvoiceCoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case MNengineRounds:
            ...
            break;
        case AnonymizeInvoiceCoinAmount:
            nAnonymizeInvoiceCoinAmount = value.toInt();
            settings.setValue("nAnonymizeInvoiceCoinAmount", nAnonymizeInvoiceCoinAmount);
            emit AnonymizeInvoiceCoinAmountChanged(nAnonymizeInvoiceCoinAmount);
            break;
        default:
            break;
        }
