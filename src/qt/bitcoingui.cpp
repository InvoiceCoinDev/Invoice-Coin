 ...
 * The InvoiceCoin Developers 2011-2012
 */
...
InvoiceCoinGUI::InvoiceCoinGUI(QWidget *parent):
    QMainWindow(parent),
    clientModel(0),
    walletModel(0),
    toolbar(0),
...
{
    resize(900, 520);
    setWindowTitle(tr("Invoice-Coin") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
#endif
    setObjectName("Invoice-Coin");
    setStyleSheet("#Invoice-Coin { background-color: #ffffff; color: #44465d;}");
...
void InvoiceCoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a Invoice-Coin address"));
    ...
    aboutAction = new QAction(QIcon(":icons/invoice-coin"), tr("&About Invoice-Coin"), this);
    aboutAction->setToolTip(tr("Show information about Invoice-Coin"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/qt-project.org/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for Invoice-Coin"));
    ...
    editConfigAction = new QAction(QIcon(":/icons/editconf"), tr("&Edit Invoice-Coin.conf"), this);
    editConfigAction->setToolTip(tr("Edit the configuration file for Invoice-Coin"));
    editConfigExtAction = new QAction(QIcon(":/icons/editconf"), tr("&Edit Invoice-Coin.conf (external)"), this);
    editConfigExtAction->setToolTip(tr("Edit the configuration file for Invoice-Coin (external editor)"));
    openDataDirAction = new QAction(QIcon(":/icons/folder"), tr("&Open data dir"), this);
    openDataDirAction->setToolTip(tr("Open the directory where Invoice-Coin data is stored"));
...
void InvoiceCoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void InvoiceCoinGUI::createToolBars()
{
...
void InvoiceCoinGUI::setClientModel(ClientModel *clientModel)
{
    if(!fOnlyTor)
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Invoice-Coin client") + QString(" ") + tr("[testnet]"));
...
void InvoiceCoinGUI::setWalletModel(WalletModel *walletModel)
{
...
void InvoiceCoinGUI::createTrayIcon()
{
    QMenu *trayIconMenu;
#ifndef Q_OS_MAC
    ...
    trayIcon->setToolTip(tr("Invoice-Coin client"));
...
#ifndef Q_OS_MAC
void InvoiceCoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void InvoiceCoinGUI::optionsClicked()
{
...
void InvoiceCoinGUI::aboutClicked()
{
    AboutDialog dlg;
...
void InvoiceCoinGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Invoice-Coin network", "", count));
}

void InvoiceCoinGUI::setNumBlocks(int count)
{
    QString tooltip;
...
void InvoiceCoinGUI::message(const QString &title, const QString &message, bool modal, unsigned int style)
{
    QString strTitle = tr("Invoice-Coin") + " - ";
...
void InvoiceCoinGUI::error(const QString &title, const QString &message, bool modal)
{
...
void InvoiceCoinGUI::changeEvent(QEvent *e)
{
...
void InvoiceCoinGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void InvoiceCoinGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;
    ...
    QString strMessage = tr("This transaction is over the size limit. You can still send it for a fee of %1, "
        ...
        "Do you want to pay the fee?").arg(InvoiceCoinUnits::formatWithUnit(clientModel->getOptionsModel()->getDisplayUnit(), nFeeRequired));
...
void InvoiceCoinGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
    ...
    notificator->notify(Notificator::Information,
                        (amount)<0 ? tr("Sent transaction") :
                          ...
                          tr("Date: %1\n"
                             ...
                             "Address: %4\n")
                          .arg(date)
                          .arg(InvoiceCoinUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(), amount, true))
                          .arg(type)
                          .arg(address), icon);
}

void InvoiceCoinGUI::clearWidgets()
{
...
void InvoiceCoinGUI::gotoMasternodeManagerPage()
{
...
void InvoiceCoinGUI::gotoBlockBrowser()
{
...
void InvoiceCoinGUI::gotoOverviewPage()
{
...
void InvoiceCoinGUI::gotoHistoryPage()
{
...
void InvoiceCoinGUI::gotoAddressBookPage()
{
...
void InvoiceCoinGUI::gotoReceiveCoinsPage()
{
...
void InvoiceCoinGUI::gotoSendCoinsPage()
{
...
void InvoiceCoinGUI::gotoSignMessageTab(QString addr)
{
...
void InvoiceCoinGUI::gotoVerifyMessageTab(QString addr)
{
...
void InvoiceCoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void InvoiceCoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Invoice-Coin address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void InvoiceCoinGUI::handleURI(QString strURI)
{
    // URI has to be valid
...
void InvoiceCoinGUI::setEncryptionStatus(int status)
{
...
void InvoiceCoinGUI::encryptWallet()
{
    if(!walletModel)
        return;
...
void InvoiceCoinGUI::backupWallet()
{
...
void InvoiceCoinGUI::importPrivateKey()
{
...
void InvoiceCoinGUI::changePassphrase()
{
...
void InvoiceCoinGUI::unlockWallet()
{
    if(!walletModel)
        return;
...
void InvoiceCoinGUI::lockWallet()
{
    if(!walletModel)
        return;
...
void InvoiceCoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void InvoiceCoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void InvoiceCoinGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void InvoiceCoinGUI::updateStakingIcon()
{
    updateWeight();
...
void InvoiceCoinGUI::detectShutdown()
{
    if (ShutdownRequested())
        QMetaObject::invokeMethod(QCoreApplication::instance(), "quit", Qt::QueuedConnection);
}

void InvoiceCoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void InvoiceCoinGUI::editConfig()
{
...
void InvoiceCoinGUI::editConfigExt()
{
...
void InvoiceCoinGUI::openDataDir()
{
