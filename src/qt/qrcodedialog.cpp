...
{
    QString ret = QString("Invoice-Coin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non IVC unit input in lnReqAmount, we generate the URI with IVC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(InvoiceCoinUnits::format(InvoiceCoinUnits::IVC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
