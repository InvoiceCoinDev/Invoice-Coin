...
{
    widget->setMaxLength(InvoiceCoinAddressValidator::MaxAddressLength);
    widget->setValidator(new InvoiceCoinAddressValidator(parent));
...
bool parseInvoiceCoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // NovaCoin: check prefix
    if(uri.scheme() != QString("Invoice-Coin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!InvoiceCoinUnits::parse(InvoiceCoinUnits::IVC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseInvoiceCoinURI(QString uri, SendCoinsRecipient *out)
{
    // Convert invoicecoin:// to InvoiceCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("invoicecoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "InvoiceCoin:");
    }
    QUrl uriInstance(uri);
    return parseInvoiceCoinURI(uriInstance, out);
}
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Invoice-Coin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Invoice-Coin.lnk
...
{
    return GetAutostartDir() / "Invoice-Coin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Invoice-Coin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Invoice-Coin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Invoice-Coin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Invoice-Coin app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Invoice-Coin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Invoice-Coin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Invoice-Coin-Qt"));
