...
//
// Bootup the masternode, look for a 10,000 IVC input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* donations are not supported in Invoice-Coin.conf */
...
{
    CInvoiceCoinAddress address(collateralAddress);
