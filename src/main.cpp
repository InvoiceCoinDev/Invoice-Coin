...
const string strMessageMagic = "Invoice-Coin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    if (nHeight > 1 && nHeight < 102) {
      nSubsidy = 270000 * COIN; // 15% Reserved for Development
                                // 153,000,000 left to mint
    }
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    // Reward calculations for 5-years of IVC emissions
    // 100% Remaining IVC   : 153,000,000
    // ----------------------------------
    // 25% for Superblocks  :  38,250,000
    // 75% for Normalblocks : 114,750,000
    // ----------------------------------
    // (COINS LEFT)       (BLOCKS | 5-Years of minting)
    // Singular Payout Example: 153000000 / (((((1 * 60 * 60) / (3 * 60)) * 24) * 365) * 5) = 174.6575 IVC per block
    // Superblock Payout: 38250000 / (((((1 * 60 * 60) / (3 * 60)) * 24) * 365) * 5) = 43.6643 IVC added for Superblock
    // Regular Payout: 114750000 / (((((1 * 60 * 60) / (3 * 60)) * 24) * 365) * 5) = 130.9931 IVC per block

    int64_t nSubsidy = 131 * COIN;
...
{
    RenameThread("Invoice-Coin-loadblk");
