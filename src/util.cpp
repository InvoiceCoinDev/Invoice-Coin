...
int nAnonymizeInvoiceCoinAmount = 1000;
...
#else
    const char* pszModule = "Invoice-Coin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\IVC
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\IVC
    // Mac: ~/Library/Application Support/IVC
    // Unix: ~/.IVC
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "IVC";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Invoice-Coin.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
    {
        boost::filesystem::path ConfPath;
               ConfPath = GetDataDir() / "Invoice-Coin.conf";
               ...
               fprintf(ConfFile, "port=10877\n");
               fprintf(ConfFile, "rpcport=10879\n");
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Invoice-Coind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Invoice-Coin will not work properly.");
